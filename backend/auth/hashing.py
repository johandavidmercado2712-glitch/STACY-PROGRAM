...
def hash_password(password: str) -> str:
    ...
    hash_obj = hashlib.pbkdf2_hmac('sha256', password.encode(), salt, 100000) #se usa el lgorithmo matematico sha256 (una fotma matematica) + convierte el texto a bytes + salt aleatoriamente +  el numero de vueltas (100000)
...
def verify_password(password: str, stored: str) -> bool: #verifica la contrasena que dijista el usuario en el login con la que se hasheo
    ...
    hash_obj = hashlib.pbkdf2_hmac('sha256', password.encode(), salt, 100000) #le aplica esto a la contrasena que dijito el usuario en el login
